...
class HydroFastAPI:
    ...
    def setup_routes(self):
        ...
        async def get_sensor(sensor_id: str):
            try:
                ...
                if sensor_id == 'ph':
                    ...
                    raise HTTPException(status_code=400, detail=f"Unknown sensor {sensor_id}")
        ...
        async def set_controller_target(controller_id: str, data: TargetValue):
            try:
                ...
                if controller_id == 'ph':
                    ...
                    raise HTTPException(status_code=400, detail=f"Unknown controller type {controller_id}")
        ...
        async def start_controller(controller_id: str, background_tasks: BackgroundTasks):
            try:
                if controller_id not in self.controllers:
                    raise HTTPException(status_code=404, detail=f"Controller {controller_id} not found")
            ...
            try:
                ...
                if 'ph' in self.sensors:
                    self.sensors['ph'].set_calibration({
                        data.voltage_1: data.ph_1,
                        data.voltage_2: data.ph_2
                    })
