...
class PHController:
    ...
    """
    def __init__(self, ph_sensor, acid_pump_pin, base_pump_pin, mosfet_control, 
                 target_ph=6.0, tolerance=0.2, check_interval=60):
        """
            ...
            if abs(current_ph - self.target_ph) > self.tolerance:
                ...
                if current_time - self.last_dose_time >= self.min_dose_interval:
                    if current_ph > self.target_ph:
                        ...
                        await self.mosfet_control.pulse(self.acid_pump_pin, self.dose_time)
                    else:
                        # pH is too low, add base
                        await self.mosfet_control.pulse(self.base_pump_pin, self.dose_time)
