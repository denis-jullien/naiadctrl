...
<div class="min-h-screen bg-background">
  <div class="flex">
    <!-- Sidebar -->
    <aside class="fixed inset-y-0 left-0 z-10 w-64 border-r bg-card hidden md:block">
            ...
            <li>
              <a 
                ...
                class="flex items-center gap-3 rounded-md px-3 py-2 text-sm transition-colors
                      {$page.url.pathname === item.href ? 'bg-accent text-accent-foreground' : 'hover:bg-accent hover:text-accent-foreground'}"
              >
                {item.label}
              </a>
            </li>
