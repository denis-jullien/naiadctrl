...
export const api = {
	...
	// Outputs (GPIO pins)
	outputs: {
		getAll: () => fetchApi<Record<string, boolean>>('/api/outputs/'),
		get: (pinNumber: number) => fetchApi<boolean>(`/api/outputs/${pinNumber}`),
		set: (pinNumber: number, state: boolean) => fetchApi<any>(`/api/outputs/${pinNumber}/set/${state}`, {
			method: 'POST',
		}),
	},
};